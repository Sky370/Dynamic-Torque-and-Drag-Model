...
def Main_Func(t, x, constants, p, fric_mod):
    ...
    # v[0] = ROP_top_drive
    # omega[0] = RPM_top_drive

    Forcing_F[-1] = -clc.K_WOB * doc  # - c_bit_axial * v[-1] * abs(np.sign(doc))
    Forcing_T[-1] = -clc.K_TQ * doc

    Friction_force, Friction_torque, p['STATIC_CHECK_PREV'], Friction_force_tors = Friction(
    ...
    p['SURFACE_WEIGHT'].append(-clc.ka[0] * (z[0] - z[1]) + sum(clc.bw_pipe * clc.global_length_array * np.cos(clc.inc_rad[1:])) + clc.TP_w)
    # p['SURFACE_WEIGHT'].append(-clc.ka[0] * (z[0] - z[1]))
    p['DOWNHOLE_TORQUE'].append(Forcing_T[-1])
    p['SURFACE_TORQUE'].append(clc.kt[0] * (theta_top_drive - theta[0]))
    p['DOC'].append(doc)
    p['SOLUTION_TIME'].append(t)
    p['FRICTION_FORCE_TORSIONAL_STORE'].append(Friction_force_tors[-1])
    ...
    dx[3::4] = J_inv.dot(Forcing_T + Friction_torque - Ct.dot(omega)*R_EQ**2 - Kt.dot(theta))
    return dx