...
class Calculations:
    def __init__(self, depth):
        # Field units
        self.g = (32.1740)  # ft/s² to m/s²
        self.bit_depth = (depth) # ft to m
        self.elem_length = float(df_ADV[df_ADV["Parameter"] == "Element Length"]["Value"])
        self.mud_density_ppg = (float(df_WELL[df_WELL["Parameter"] == "Mud Density"]["Value"]))  # ppg to lb/ft3
        self.pipe_density = (float(df_WELL[df_WELL["Parameter"] == "Steel Density"]["Value"]))   # ppg to lb/ft3
        self.visc_p = cp2lbfft2(float(df_WELL[df_WELL["Parameter"] == "Plastic Viscosity"]["Value"]))      # cP to lbfft2 
        self.tao_y = (float(df_WELL[df_WELL["Parameter"] == "Yield Point"]["Value"]))             # lbf/100ft2 to Pa
        ...
        self.G = float(df_ADV[df_ADV["Parameter"] == "Shear Modulus"]["Value"])                          # Shear Modulus
        ...
        # Velocity definition
        self.v1 = (float(df_TOP[df_TOP["Parameter"] == "Top Drive Axial Velocity Magnitude 1 (ft/min)"]["Value"])) /60    # ft/min to m/s
        self.v2 = (float(df_TOP[df_TOP["Parameter"] == "Top Drive Axial Velocity Magnitude 2 (ft/min)"]["Value"])) /60    # ft/min to m/s
        ...
        self.DP_MASS_ARRAY = np.ones(self.N_DP) * df_BHA["Mass (lbs)"].iloc[0] #* self.L_DP / df_BHA["Length (ft)"].iloc[0] 
        ...
        # BHA:
        # self.N_BHA, self.L_BHA = nearestLength(self.bha_length, self.elem_length)
        self.N_BHA = np.ceil(df_BHA["Length (ft)"] / self.elem_length)
        self.BHA_TYPES = np.array(np.repeat(df_BHA["BHA Type"], self.N_BHA ))
        self.BHA_LEN = np.array(np.repeat(df_BHA["Length (ft)"]/self.N_BHA  , self.N_BHA ))
        self.BHA_MASS = np.array(np.repeat(df_BHA["Mass (lbs)"]/self.N_BHA  , self.N_BHA ))
        self.BHA_OD = np.array(np.repeat(df_BHA["OD (in)"], self.N_BHA ))
        self.BHA_ID = np.array(np.repeat(df_BHA["ID (in)"], self.N_BHA )) 
        ...
        # Bottom hole
        self.HOLE_OD = (df_ADV[df_ADV["Parameter"] == "Hole Diameter"]["Value"].iloc[0])/12    # inch to ft
        self.HOLE_DEPTH = (df_ADV[df_ADV["Parameter"] == "Hole Depth"]["Value"].iloc[0])    # ft
        ...
        # Concatenation
        self.global_mass_array = (np.concatenate([self.DP_MASS_ARRAY, self.BHA_MASS]))/ self.g     # Final conversion from lbs to kg
        self.global_length_array = np.concatenate([self.L_DP_ARRAY, (self.BHA_LEN)])         # ft to m
        self.global_od_array = (np.concatenate([self.DP_OD, self.BHA_OD]))/12                   # inch to ft
        self.global_id_array = (np.concatenate([self.DP_ID, self.BHA_ID]))/12                   # inch to ft
        ...
        self.J_m = self.rho * self.J_polar * self.global_length_array
        ...
        self.inc, self.azi, self.K = survey_mod_IMPERIAL(df_SRV, (self.MD), self.bf, self.global_mass_array, self.g)
        # self.K *= 0.3048 # deg/m to deg/ft
        ...
        self.DIA_EQ = ((27*(self.global_od_array)*12 + 3*self.TJ_OD) / 30)/12     # in to ft
